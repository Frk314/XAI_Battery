...
# %%
def make_gradcam_heatmap(img_array, model, last_conv_layer_number, pred_index=None):
    ...
    grad_model = keras.models.Model(
        model.inputs, [model.layers[last_conv_layer_number].output, model.output]
    )
...
for n in range(1, 11):
    ...
    mdir = f'./new_cnn_models/{dataset_name}/'
    rdir = f'./new_cnn_results/'
    # model = tf.keras.models.load_model(mdir+f'{model_name}/{model_name}.keras')
    model.load_weights(mdir+f'{model_name}/{model_name}.weights.h5')
    ...
    for i, data in enumerate(test_data_tf):
        ...
        heatmap = make_gradcam_heatmap(test_data_tf[i], model, 4)
        hm.append(heatmap)
