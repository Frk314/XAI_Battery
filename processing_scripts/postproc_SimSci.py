...
# transformer 
mdir = './transformer_results/matr_1/'

model_name = 'transformers_1'
data_tb = np.load(mdir+f'res_{model_name}.npz')

model_name = 'transformers_8'
data_tw = np.load(mdir+f'res_{model_name}.npz')

# cnn
mdir = './cnn_results/matr_1/'

model_name = 'model_2'
data_cb = np.load(mdir+f'res_{model_name}.npz')

model_name = 'model_5'
data_cw = np.load(mdir+f'res_{model_name}.npz')


rmse_cb = rmse(data_cb)
rmse_cw = rmse(data_cw)

rmse_tb = rmse(data_tb)
rmse_tw = rmse(data_tw)
...
def plot_best_worst(data_best, data_worst, error_best, error_worst, index, name='qm_heatmaps', save=False):
    ...
    # plt.set_cmap('plasma')
    c1 = ax[1].pcolormesh(xxh, yyh, hmb)
    ...
    c2 = ax[2].pcolormesh(xxh, yyh, hmw)
...
#     plot_best_worst(data_cb, data_tb, e_cb, e_tb, index, name=f'best_{index}', save=True)
#     plot_best_worst(data_cw, data_tw, e_cw, e_tw, index, name=f'worst_{index}', save=True)
...
for i in range(1, 11):
    ...
    data = np.load(mdir+f'res_{model_name}.npz')
...
plot_rul(ref, pred, std, 'cnn', save=False)
...
plot_rul(ref, pred, std, 'transformer', save=False)