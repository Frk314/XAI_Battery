...
#We resize our data in the form of patches
    ...
    def on_epoch_end(self, epoch, logs=None):
        if epoch != 0 and epoch % 499 == 0:  # Run only every 100 epochs
            ...
            for idx, img in enumerate(self.sample):
                ...
                roi_1 = heatmap[80:320, 26:]  # Extract the region with the charge voltage depression
                roi_2 = heatmap[80:320, :26]  # Extract the region adjacent to it for comparisom
            ...
            if perf_values:  # Ensure non-empty list before computing overall average
                ...
                if perf_avg < self.threshold or math.isnan(perf_avg):
...
def make_gradcam_heatmap(img_array, model, last_conv_layer_name, alpha=0.1, pred_index=None):
    ...
    tf.where(heatmap > 0, heatmap, heatmap * alpha)  # Leaky ReLU
...
def reset_model():
    mod = visualTransformer()
    return compile_model(mod)
...
model = compile_model(visualTransformer())


tries = 40  # Maximum number of training attempts
current_try = 0  # Track number of attempts
total_epochs_trained = 0  # Track total epochs trained
threshold_value = 0.52   # Define the threshold and the name of the convolutional layer to inspect.
...
with PdfPages('visual_transformer/new_models/{}/Result.pdf'.format(m)) as pdf:
...
for i, data in enumerate(test_data_tf):
        ...
        with open('visual_transformer/new_models/{}/heatmap_{}.pkl'.format(m, i), 'wb') as f:  # Python 3: open(..., 'wb')
